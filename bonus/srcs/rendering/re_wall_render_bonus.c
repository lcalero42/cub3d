...
/*   Updated: 2025/07/31 16:03:42 by ekeisler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void calculate_wall_bounds(t_data *data, double perp_wall_dist,
	...
	*draw_end = line_height / 2 + WINDOW_HEIGHT / 2 + (int)pitch_offset;
	...
	if (*draw_start >= WINDOW_HEIGHT) *draw_start = WINDOW_HEIGHT - 1;
...
static int calculate_texture_x(t_data *data, int x, double perp_wall_dist) {
	...
	if (data->rays[x].side == 0)
		wall_x =
		    data->player.position.y + perp_wall_dist * data->rays[x].ray_dir.y;
	else
		wall_x =
		    data->player.position.x + perp_wall_dist * data->rays[x].ray_dir.x;
	wall_x -= floor(wall_x);
	tex_x = (int)(wall_x * 64);
...
static void draw_wall_column(t_data *data, int x, int draw_start,
	...
	tex_pos = (draw_start - original_wall_center - (int)pitch_offset +
	           (WINDOW_HEIGHT / perp_wall_dist) / 2) *
	          step;
	y = draw_start;
	while (y < draw_end) {
		put_pixel_to_image(
