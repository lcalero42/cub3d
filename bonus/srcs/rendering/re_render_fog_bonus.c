...
/*   Updated: 2025/07/31 16:07:45 by ekeisler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
                                  ...
                                  t_wall_bounds *bounds)
{
	...
	bounds->wall_start =
	    -wall_height / 2 + WINDOW_HEIGHT / 2 + (int)pitch_offset;
	bounds->wall_end = wall_height / 2 + WINDOW_HEIGHT / 2 + (int)pitch_offset;
	if (bounds->wall_start < 0) bounds->wall_start = 0;
	if (bounds->wall_end >= WINDOW_HEIGHT) bounds->wall_end = WINDOW_HEIGHT - 1;
	if (bounds->wall_start >= WINDOW_HEIGHT)
		bounds->wall_start = WINDOW_HEIGHT - 1;
	if (bounds->wall_end < 0) bounds->wall_end = 0;
}
...
static void process_fog_column(t_data *data, int x)
{
	...
	if (params.fog_alpha > 0) {
		...
		params.fog_color =
		    u_rgb_to_hex(FOG_COLOR_R, FOG_COLOR_G, FOG_COLOR_B, 255);
