...
/*   Updated: 2025/12/04 18:24:01 by lcalero          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return ((int)(256 * (x - ((-calc->sprite_width >> 1) + calc->screen_x))
		* 64 / calc->sprite_width) / 256);
}

static int	get_tex_y(t_sprite_calc *calc, int y, int sprite_center_y)
{
	return (((y - sprite_center_y + (calc->sprite_height >> 1))
	...
	t_texture_info *tex, int x)
{
	...
	sprite_center_y = (WINDOW_HEIGHT >> 1) + (int)data->player.pitch_offset;
	coords[0] = x;
